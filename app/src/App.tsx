...
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxYg61NyShuQS6Llh1s2BS10GFWeS0aqwGooKRpJ4ln6k6_-AfYq0vuQZV0XgaydXHUKA/exec';
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 bg-pattern">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isScrolled ? 'header-blur shadow-lg py-3' : 'bg-transparent py-5'
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 animate-fadeIn">
              ...
              <span className="font-bold text-lg text-slate-800">{t.headerTitle}</span>
            </div>
            <Button
              variant="ghost"
              size="sm"
              onClick={toggleLanguage}
              className="flex items-center gap-2 hover:bg-blue-100/50 transition-all duration-300"
            >
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="glass border-0 shadow-2xl overflow-hidden animate-fadeInUp delay-300">
            ...
            <CardHeader className="text-center pb-2">
              ...
              <CardTitle className="text-2xl md:text-3xl font-bold text-slate-800">{t.formTitle}</CardTitle>
            ...
            <CardContent className="p-6 md:p-8">
              {isSuccess ? (
                <div className="text-center py-8 animate-success">
                  ...
                  <h3 className="text-2xl font-bold text-slate-800 mb-3">{t.successTitle}</h3>
                  <p className="text-slate-600 mb-8 max-w-md mx-auto">{t.successMessage}</p>
                  <Button
                    onClick={handleReset}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {/* Complaint Type */}
                  <div className="space-y-2">
                    <Label htmlFor="complaintType" className="text-sm font-semibold text-slate-700 flex items-center gap-2">
                  ...
                  {/* Debug Info */}
                  {debugInfo && (
                    <Alert className="bg-amber-50 border-amber-200 animate-fadeIn">
                      <AlertDescription className="text-amber-800 text-sm font-mono">
                  ...
                  <Button
                    ...
                    className="w-full h-14 text-lg font-semibold bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white rounded-xl btn-hover-lift animate-pulse-glow"
                  >
                    {isSubmitting ? (
          ...
          <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: Star, title: language === 'ar' ? 'جودة عالية' : 'High Quality', desc: language === 'ar' ? 'نحن نسعى دائماً لتقديم أفضل الخدمات' : 'We always strive to provide the best services' },
              { icon: Shield, title: language === 'ar' ? 'أمان البيانات' : 'Data Security', desc: language === 'ar' ? 'بياناتك محمية ومشفرة بأعلى معايير الأمان' : 'Your data is protected with highest security standards' },
              { icon: Headphones, title: language === 'ar' ? 'دعم 24/7' : '24/7 Support', desc: language === 'ar' ? 'فريق الدعم جاهز لمساعدتك على مدار الساعة' : 'Support team ready to help you around the clock' },
            ].map((feature, index) => (
